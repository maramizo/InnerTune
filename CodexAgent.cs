...
public sealed class CodexAgent
{
    ...
    public async Task<string> RunAsync(string request, string dataDirectory, CancellationToken token = default)
    {
        ...
        const string prompt = """
            ...
            Inspect state and search iteratively. Adding, replacing, or loading a queue NEVER starts playback or interrupts the current song.
            Only call control_playback when explicitly asked. Match requested counts exactly and avoid duplicates unless asked.
