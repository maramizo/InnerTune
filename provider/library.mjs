...
export class LibraryStore {
  ...
  findQueue(data, key) { const value = key.toLowerCase(); const matches = data.savedQueues.filter(x => x.id === key || x.name.toLowerCase() === value || this.queuePath(data, x).toLowerCase() === value); if (matches.length !== 1) throw new Error(matches.length ? 'Queue name is ambiguous; use its full path.' : 'Saved queue not found.'); return matches[0]; }
}
